...
enum File { A, B, C, D, E, F, G, H, NO_FILE };
enum Rank { First, Second, Third, Fourth, Fifth, Sixth, Seventh, Eighth, NO_RANK };
...
inline File get_file(Square square) {
    if (square == Square::EMPTY_SQUARE) return File::NO_FILE;
