...
/*
Perft is a recursive debugging function which makes it easier to find obscure
edge cases and bugs in move generation.
...
inline int perft(Position& pos, int depth) {
    ...
    for (auto move : moves.move_list()) {
        ...
        if (isValidMove) {
            ...
            bool isValidGameHistory =
                (initial_game_history_size == current_game_history_size);
            if (!undo || !isValidGameHistory) {
                throw std::invalid_argument("can't undo.");
            }
        }
    }
    bool gameHistoryIsConsistent =
        (pos.gameHistory.size() == initalGameHistory);
    if (!gameHistoryIsConsistent)
        throw std::invalid_argument("inconsistent game history");
    return nodes;
}
...
inline std::vector<Output> perft_verbose(Position& pos, int depth) {
    ...
    // static_cast<U64>(n_moves.move_list().size())}};
