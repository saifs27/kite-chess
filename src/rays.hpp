...
namespace Rays
{
    ...
    inline static constexpr auto rayAttacks = []()
    {
        ...
        for (int sq = 0; sq < 64; sq++, east <<=1, west >>=1) {
            ...
            rays[63-sq][static_cast<int>(Direction::WEST)] = west & get_rank_mask<int>(63-sq);
        }
        ...
        for (int f = 0; f < 8; f++, ne <<= 1, nw >>=1) {
            ...
            for (int r = 0; r < 64; r += 8, northeast <<= 8, northwest <<= 8) {
                rays[r+f][static_cast<int>(Direction::NE)] = northeast & mask_east(r+f);
                rays[r+(7-f)][static_cast<int>(Direction::NW)] = northwest & mask_west(r+(7-f));
            } 
        }
        ...
        for (int f = 0; f < 8; f++, se <<= 1, sw >>=1) {
            ...
            for (int r = 56; r >= 0; r -= 8, southeast >>=8, southwest >>=8) {
                rays[r+f][static_cast<int>(Direction::SE)] = southeast & mask_east(r+f);
                rays[r+(7-f)][static_cast<int>(Direction::SW)] = southwest & mask_west(r+(7-f));
            } 
        }
