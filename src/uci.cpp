...
std::optional<Move> uci_to_move(Position& pos, std::string uci) {
    ...
    if (uci.size() == 5) {
        ...
        switch (promoted) {
            case Piece::QUEEN:
                promoteFlag = Flag::PROMOTE_QUEEN;
                break;
            case Piece::ROOK:
                promoteFlag = Flag::PROMOTE_ROOK;
                break;
            case Piece::KNIGHT:
                promoteFlag = Flag::PROMOTE_KNIGHT;
                break;
            case Piece::BISHOP:
                promoteFlag = Flag::PROMOTE_BISHOP;
                break;
            default:
                return {};
        }
    ...
    if (pce == Piece::KING && move.from() == Square::E1 &&
        move.to() == Square::G1) {
        move.set_flags(Flag::KING_CASTLE);
    }
    if (pce == Piece::KING && move.from() == Square::E8 &&
        move.to() == Square::G8) {
        move.set_flags(Flag::KING_CASTLE);
    }
...
void uci_loop() {
    while (true) {
        ...
        if (args.at(0).compare("uci") == 0)
            std::cout << "id name " << engine_version << '\n' << "uciok";

        else if (args.at(0).compare("isready") == 0)
            std::cout << "readyok";

        else if (args.at(0).compare("quit") == 0)
            break;
